const objectPaths = new Map<object, string>();
...
export function inspect(value: any, indent = 2, wrapAt = 80): string {
  try {
    return inspectValue(value, '#', '', 0, indent, indent === 2 ? '  ' : ' '.repeat(indent), wrapAt);
  } finally {
    objectPaths.clear();
  }
}
...
function inspectValue(
  ...
  if (Array.isArray(value)) {
    for (let i = 0; i < value.length; i++) {
      const chunk = inspectValue(value[i], parentPath, i, depth + 1, indent, space, wrapAt);
    ...
    for (const key in value) {
      ...
      const chunk = keyChunk + ': ' + inspectValue(value[key], parentPath, keyChunk, depth + 1, indent, space, wrapAt);
